...
{
    public partial class AdminOwnerForm : Form
    {
        public AdminOwnerForm()
        {
            InitializeComponent();
        }
        public AdminOwnerForm(AdminEntity admin)
        {
            InitializeComponent();
            this.admin = admin;
            page.PageNum = 1;
        ...
        private void get(Page page)
        {
            ...
            r = ownerMapper.selectByTable(page, o_sex, o_tel, o_point);
            if (r.IsOK)
            {
        ...
        private void dataGridView1_CellContentClick_1(object sender, DataGridViewCellEventArgs e)
        {
            ...
            index = e.RowIndex;
            string o_id = dataGridView1.Rows[e.RowIndex].Cells["房主ID"].Value.ToString();
            ...
            if (e.ColumnIndex == 0)
            {
                ...
                {
                    r = ownerMapper.updateStateById(o_id, 1);
                    warn_label.Text = r.Msg;
                    if(r.IsOK) 
                ...
                {
                    r = ownerMapper.updateStateById(o_id, 0);
                    warn_label.Text = r.Msg;
                    if (r.IsOK)
            ...
            else
            {
                ...
                owner.O_id = o_id;
                owner.O_state = state;
                owner.O_sex = dataGridView1.Rows[e.RowIndex].Cells["性别"].Value.ToString();
                owner.O_tel = dataGridView1.Rows[e.RowIndex].Cells["电话"].Value.ToString();
                owner.O_name = dataGridView1.Rows[e.RowIndex].Cells["姓名"].Value.ToString();
                owner.O_point = Convert.ToInt32(dataGridView1.Rows[e.RowIndex].Cells["积分"].Value);
                AdminPointForm adminPoint = new AdminPointForm(this, null,owner) ;
